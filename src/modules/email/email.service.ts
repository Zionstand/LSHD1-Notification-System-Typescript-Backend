...
// Use require for node-mailjet due to ES module compatibility issues
...
export class EmailService {
  ...
  constructor(private configService: ConfigService) {
    const publicKey = this.configService.get<string>('MAILJET_API_PUBLIC_KEY');
    const privateKey = this.configService.get<string>('MAILJET_API_PRIVATE_KEY');
    ...
    this.senderEmail = this.configService.get<string>('SENDER_EMAIL_ADDRESS') || 'noreply@lshd1.com';
    this.senderName = this.configService.get<string>('SENDER_EMAIL_NAME') || 'LSHD1 Screening System';
    this.adminEmail = this.configService.get<string>('ADMIN_EMAIL_ADDRESS') || '';
    this.supportEmail = this.configService.get<string>('SUPPORT_EMAIL_ADDRESS') || '';
    this.supportPhone = this.configService.get<string>('SUPPORT_PHONE_NUMBER') || '';
  }
  ...
  /**
   * Send email to staff when their account is approved
   */
  async sendStaffApprovalEmail(
    staffEmail: string,
    staffName: string,
    role: string,
  ): Promise<EmailResult> {
...
${this.supportPhone ? `Phone: ${this.supportPhone}` : ''}
  ...
  /**
   * Send email to staff when their account is rejected
   */
...
${this.supportPhone ? `Phone: ${this.supportPhone}` : ''}
  ...
  /**
   * Send email to admin when new staff registers
   */
